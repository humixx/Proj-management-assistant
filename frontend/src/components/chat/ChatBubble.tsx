...
export default function ChatBubble({ projectId }: ChatBubbleProps) {
  ...
  return (
    <>
      ...
      {/* Chat Panel */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-96 h-[520px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden animate-in">
          {/* Header */}
          <div className="px-4 py-3 border-b bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex-shrink-0">
          ...
          {/* Error bar */}
          {error && (
            <div className="px-3 py-1.5 bg-red-50 border-b border-red-200 flex justify-between items-center flex-shrink-0">
              <span className="text-xs text-red-700">{error}</span>
              <button onClick={clearError} className="text-xs text-red-500 hover:text-red-700">Dismiss</button>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {messages.length === 0 && !isSending ? (
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <p className="text-xs text-gray-500 mt-1">Ask me to create tasks or manage your project</p>
              </div>
            ) : (
              <>
                {messages.map((msg) => (
